...
import type { Paper, Question, NumberingFormat, MainNumberingFormat } from './page';
...
export const renderQuestionContent = (question: Question, questionIndex: number, allQuestions: Question[], showMainContent: boolean, subQuestionsToRender: Question[] = [], mainNumberingFormat: MainNumberingFormat = 'english-numeric') => {
    ...
    return (
      <div key={`${question.id}-${showMainContent}-${subQuestionsToRender.map(sq => sq.id).join('-')}`} className="mb-2 question-item" data-question-id={question.id}>
        ...
        {(question.type === 'table' || question.type === 'fill-in-the-blanks') && question.showHints !== false && question.tableData && question.tableData.length > 0 && (
          <div data-table-element style={{ marginTop: '0.5rem' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                {question.tableData.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    {row.map((cell, colIndex) => (
                      <td key={colIndex} style={{ border: '1px solid #000', padding: '4px 8px', verticalAlign: 'top' }}>
              ...
              return (
                <div key={sq.id} className="subquestion-item pt-1" data-subquestion-id={sq.id}>
                  <div className="flex justify-between">
                    <p>{getNumbering(mainQuestionObj?.numberingFormat, sqIndex)}) <RichText content={sq.content} /></p>
                  ...
                  {sq.options && sq.options.length > 0 && (
                    <div className="pl-6 mt-1 grid grid-cols-2 gap-x-8 gap-y-1">
                      {sq.options.map((option, optIndex) => (
                        <p key={option.id}>{getNumbering('bangla-alpha', optIndex)}) <RichText content={option.text} /></p>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div ref={ref} className="bg-white text-black font-serif max-w-none mx-auto shadow-lg paper-page" style={pageStyle}>
            ...
            <main className={!isFirstPage ? 'pt-8' : ''}>
                {pageContent.map(content => {
                    ...
                    return renderQuestionContent(content.mainQuestion, questionNumber, allQuestions, content.showMainContent, content.subQuestions, paper.mainNumberingFormat);
                })}
            </main>
        </div>
    );
